...
update_gluetools_config() {
    ...
    config_file=$(pwd)/glue_jar_files/"gluetools-config.xml"
...
download_javafx_sdk() {
    local javafx_url=$1
    local javafx_zip="javafx-tmp.zip"

    echo "Downloading JavaFX SDK from $javafx_url"
    curl -L -o $javafx_zip $javafx_url

    echo "Unzipping JavaFX SDK..."
    unzip $javafx_zip -d $(pwd)/javafx-tmp

		mv $(pwd)/javafx-tmp/java* $(pwd)/javafx-sdk
		rm -r $(pwd)/javafx-tmp
		rm $(pwd)/javafx-tmp.zip

    echo "JavaFX SDK installed at $(pwd)/javafx-sdk"
}

# Detect OS and download the appropriate JavaFX SDK
if [[ "$os_type" == "Linux" ]]; then
    echo "Linux detected. Downloading JavaFX SDK for Linux..."
    download_javafx_sdk "https://download2.gluonhq.com/openjfx/21.0.4/openjfx-21.0.4_linux-x64_bin-sdk.zip"

elif [[ "$os_type" == "Darwin" && "$architecture" == "arm64" ]]; then
    echo "macOS on ARM architecture detected. Downloading JavaFX SDK for macOS (ARM64)..."
    download_javafx_sdk "https://download2.gluonhq.com/openjfx/21.0.4/openjfx-21.0.4_osx-aarch64_bin-sdk.zip"

elif [[ "$os_type" == "Darwin" && "$architecture" == "x86_64" ]]; then
    echo "macOS on x86_64 architecture detected. Downloading JavaFX SDK for macOS (x86_64)..."
    download_javafx_sdk "https://download2.gluonhq.com/openjfx/21.0.4/openjfx-21.0.4_osx-x64_bin-sdk.zip"

else
    echo "Unsupported OS type or architecture for JavaFX SDK download."
fi
